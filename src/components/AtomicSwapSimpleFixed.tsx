...
export const AtomicSwapSimple: React.FC = () => {
  ...
  const switchNetwork = async (networkName: NetworkName) => {
    ...
    try {
      ...
      // Wait for network switch with retry
      let attempts = 0;
      ...
      while (attempts < maxAttempts) {
        ...
        try {
          const updatedNetwork = await wallet.provider.getNetwork();
      ...
      addLog(`Network switch may have completed. Please verify manually.`, 'info');
    } catch (error: any) {
      if (error.code === 4902) {
        ...
        addLog(`Network switch in progress. Please wait and try again in a moment.`, 'error');
      } else {
  ...
  const withdrawFromBob = async () => {
    ...
    try {
      ...
      const tx = await contract.withdraw(swap.bobContract, swap.secret);
      addLog(`Transaction submitted: ${tx.hash}`, 'info');
